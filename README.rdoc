...
Provides a means of contextually associating sets of ActiveRecord models with database tables

This can be used to maintain multiple independent versions of model data, for applications such as multi-tenancy

Additionally, each model participating in a ModelSpace can optionally be assigned a number of "history versions" which are consistent historical versions of all the models in a ModelSpace, and methods are provided to create new or updated versions of models in a ModelSpace
...
Include the <tt>ActiveRecord::ModelSpaces</tt> module in a model Class, and declare the model to be part of a ModelSpace, optionally specifying a number of history versions (which defaults to 0)
...
At this point, the model is no longer usable outside of a ModelSpaces context. To establish a context use the <tt>with_model_space_context</tt> method, which provides a key to be used along with the ModelSpace name to generate the name of the database table to be used by the model for the duration of the context

  AModel.table_name # RuntimeException => "ModelSpace 'AModel' has no current context"

  ActiveRecord::ModelSpaces.with_model_space_context(:metadata, :one) do
    AModel.table_name # "metadata__one__a_model"
  end
...
If a model is declared as having history_versions then historical copies of the tables will be maintained. New or updated versions of a model can be created using the new_version and updated_version class methods on the model Class, while a context for the ModelSpace is established. The new versions are only visible within the context that they are created in, until the context complete without Error, at which point any new context established for that ModelSpace will use the newly created versions. Table names wrap once the given number of history version tables have been used
  ...
  ActiveRecord::ModelSpaces.with_model_space_context(:metadata, :one) do
    ...
    AModel.new_version do
      AModel.table_name # "metadata__one__a_model__1"
    end

    AModel.table_name # "metadata__one__a_model__1"

    BModel.table_name # "metadata__one__b_model"

    BModel.updated_version do
      BModel.table_name # "metadata__one__b_model__1"
    end
